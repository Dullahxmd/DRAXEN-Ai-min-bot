const axios = require('axios');
  ...
  const FOOTER = '\n\n> Draxen is fast';
  const BACKEND_URL = 'https://backend-c-9b62b7ce63dc.herokuapp.com';
  ...
  module.exports = {
      ...
      async execute(socket, msg) {
          ...
          await socket.sendMessage(
              msg.key.remoteJid,
              { text: `Generating pairing code for *${targetNumber}*... Please wait.${FOOTER}` },
              { quoted: fakeQuoted }
          );
          ...
          try {
              ...
              if (!pairingCode) {
                  await socket.sendMessage(
                      msg.key.remoteJid,
                      { text: 'Could not generate a code right now. All servers may be busy. Please try again in a moment.' + FOOTER },
                      { quoted: fakeQuoted }
                  );
                  return;
              }
