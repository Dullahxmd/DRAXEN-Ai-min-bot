...
module.exports = {
    ...
    async execute(socket, msg, number, config, loadUserConfigFromMongo, activeSockets, socketCreationTime, extras) {
        ...
        if (!isBotAdmin) return socket.sendMessage(from, { text: '*Make me admin first before I can demote anyone.*' + FOOTER }, { quoted: fakeQuoted });
        if (!isAdmin && !isDev) return socket.sendMessage(from, { text: '*Only admins can use demoteall.*' + FOOTER }, { quoted: fakeQuoted });
        ...
        if (toDemote.length === 0) {
            return socket.sendMessage(from, { text: '*No admins to demote. Already a flat hierarchy.*' + FOOTER }, { quoted: fakeQuoted });
        }
